...
public class EvolveBlend {
	...
	/**
	 * This method takes two sets of genes and for each one takes an average of the mothers gene value
	 * and the fathers and adds it to an array of numbers which are given to a new child biomorph as gene
	 * values.
	 * @return A Biomorph with gene values as described above.
	 */
	public Biomorph evolve(){
		
		//Each gene value from the mother and father is pulled out averaged and placed in the childGenes array.
		for(int i = 0; i<fatherGenes.length; i++){
			childGenes[i] = (fatherGenes[i].getValue() + motherGenes[i].getValue())/2;
		}
		
		//A new biomorph is created using the values taken from the parents above.
