 package biomorphHandling;
import org.lwjgl.opengl.GL11;
import genes.*;
import geometry.*;
/**
 * Class to model and draw a single biomorph.
 * @author Tom Connolly, Jack Taylor
 * @version 24/02/2015
 */
public class Biomorph
{
	// An array is instantiated to hold 11 Genes that will be defined in the
	// constructor.
	private Gene[] genes = new Gene[10];
	public Biomorph(int branchVal, int branchIncrementVal, int chainVal, int redVal, int greenVal, int blueVal, int lengthVal, int lengthIncrementVal, int thicknessVal, int thicknessIncrementVal)
	{
		/*
		 * Genes are created and assigned values according to the parameters
		 * given. They are then placed in an array in alphabetical (apart from
		 * colours which are RGB) order.
		 */
		genes[0] = new Branch(branchVal, "Branch");
		genes[1] = new BranchIncrement(branchIncrementVal, "Branch Increment");
		genes[2] = new Chain(chainVal, "Chain");
		genes[3] = new Color(redVal, "Color Red");
		genes[4] = new Color(greenVal, "Color Green");
		genes[5] = new Color(blueVal, "Color Blue");
		genes[6] = new Length(lengthVal, "Length");
		genes[7] = new LengthIncrement(lengthIncrementVal, "Length Increment");
		genes[8] = new Thickness(thicknessVal, "Thickness");
		genes[9] = new ThicknessIncrement(thicknessIncrementVal, "Thickness Increment");
	}
	/**
	 * Draws this biomorph.
	 */
	public void draw()
	{
		int i = 0; 
		if(i < genes[8].getValue())
		{
			
			// Set the thickness of the branches
			GL11.glLineWidth(genes[8].getValue());
		//Enable to auto-rotate
		//GL11.glRotatef(0.5f, 1.0f, 1.0f, 1.0f);
		GL11.glPushMatrix();
			//GL11.glRotatef(45.0f, 1.0f, 1.0f, 1.0f);
			loop(genes[2].getValue(),0);
			GL11.glPushMatrix();
				GL11.glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
				loop(genes[2].getValue(),0);
				GL11.glPushMatrix();
					GL11.glRotatef(90.0f, 1.0f, 0.0f, 0.0f);
					loop(genes[2].getValue(),0);
				GL11.glPopMatrix();
			GL11.glPopMatrix();
		GL11.glPopMatrix();
		//increase the thickness of the biomorph.
				i = (genes[8].getValue() + (i+genes[10].getValue()));
			    i++;
			}
			  i = genes[9].getValue();
	}
	/**
	 * Recursive loop to draw chains from each branch.
	 * @param chains The amount of recursive steps
	 */
	private void loop(int chains, int limbCount)
	{
		int maxLength = 10;
		int length = genes[6].getValue();
		for (int b = 0; b < genes[0].getValue(); b++)
		{
			GL11.glPushMatrix();
			// This allows the distribution of branches to be uniform.
			GL11.glRotatef((float) b * (360.0f / genes[0].getValue()), 0.0f, 0.0f, 1.0f);
			/* code to update length value using length Increment  TODO: Uncomment when needed.*/
			length = (genes[6].getValue() + (limbCount*genes[7].getValue()));
			if(length<maxLength){
				length = genes[7].getValue();
			}
			new CuboidLimb(length, genes[8].getValue(), genes[3].getValue(), genes[4].getValue(), genes[5].getValue()).draw();
			GL11.glPopMatrix();
			limbCount++;
			// This draws a new set of branches from the end of each existing
			// branch if necessary.
			if (chains > 1)
			{
				GL11.glPushMatrix();
				GL11.glRotatef((float) b * (360.0f / genes[0].getValue()), 0.0f, 0.0f, 1.0f);
				GL11.glTranslatef(0.0f, (float) genes[6].getValue(), 0.0f);
				limbCount++;
				loop(chains - 1, limbCount);
				GL11.glPopMatrix();
			}
		}
		length = genes[7].getValue();
	}
	/**
	 * @return An array containing a value for each Gene of this biomorph
	 */
	public Gene[] getGenes()
	{
		return genes;
	}
}