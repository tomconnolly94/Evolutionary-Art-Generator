...
public class BiomorphCreator_Test
{
	...
	private int[] geneValues = {8,0,2,105,15,172,2,-1,2,5};
	
	@Before //pre-conditions
	public void setUp()
	{
		/*for test to pass the seed in biomorphCreator must be set to 10.
		this means the random factor of the createBiomorph class can be 
		tested.*/
		bc = new BiomorphCreator();
		biomorphRand = bc.createBiomorph();
		biomorphNonRand = bc.createBiomorph(geneValues[0],geneValues[1],geneValues[2],geneValues[3],geneValues[4],geneValues[5],geneValues[6],geneValues[7],geneValues[8],geneValues[9]);
